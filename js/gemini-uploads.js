...
9. CROSS-PAGE CONTINUATIONS — read this carefully, this is a common failure mode: treat the ENTIRE document as one continuous stream of content. Page boundaries are just where the scan/print was physically cut and carry NO semantic meaning — never let one cause you to drop, truncate, or duplicate anything.
   - A question's stem, its answer choices, and its indicated correct answer are frequently split across two (or more) pages — e.g. the stem and choices A–C end at the very bottom of one page, and choice D plus the answer marking are the very first thing at the top of the next page. This is NORMAL pagination, not a sign the question ended or that there's a new, separate question. Whenever a page ends with a question stem that has no options yet, or with an options list that looks incomplete (e.g. only A and B given, no C/D, and the question doesn't clearly end there), the continuation is almost always the first thing on the next page — go there and attach it, don't treat the cut-off point as the question's real end.
   - WORKED EXAMPLE of what you must do: suppose page 7 ends with "14. Which of the following is the most likely diagnosis?\nA. Addison disease\nB. Cushing syndrome" and page 8 begins with "C. Conn syndrome\nD. Pheochromocytoma\n(Answer: C)" — followed by unrelated question 15. The correct output for question 14 is ONE object: question "Which of the following is the most likely diagnosis?", options A–D all four listed above, answer "C". It is WRONG to output question 14 with only options A and B, WRONG to mark it "__NO_KEY__", and WRONG to drop it entirely because its answer key was on the next page.
   - The same applies when a question appears on one page and its ONLY answer indication is in a separate answer-key list positioned elsewhere in the document (a different page, or a key section at the very end, e.g. "12-C, 13-A, 14-B..."). Find that entry by matching the question number and use it — do not mark the question "__NO_KEY__" just because the key wasn't printed directly under the question itself.
   ...
   - MANDATORY SELF-CHECK BEFORE YOU OUTPUT ANYTHING: once you've made a first pass through the whole document, go through your draft list of questions again, in order, and for each one ask: (a) does its "options" object have at least 2 entries — if not, re-search the page immediately before and after where this question started, plus any answer-key section, for the missing choices before finalizing; (b) if the question numbers in the source jump (e.g. you have 13 and 15 but not 14), go find 14 — it's usually the victim of a page-break split and got attached to the wrong neighbor or missed; (c) is "answer" set to "__NO_KEY__" only because the key genuinely never appears anywhere in the document, not because you stopped looking after the page the question started on. Only finalize the JSON after this check.
