...
9. CROSS-PAGE CONTINUATIONS — read this carefully, this is a common failure mode: treat the ENTIRE document as one continuous stream of content. Page boundaries are just where the scan/print was physically cut and carry NO semantic meaning — never let one cause you to drop, truncate, or duplicate anything.
   - A question's stem, its answer choices, and its indicated correct answer are frequently split across two (or more) pages. Sometimes it's a partial split (e.g. the stem and choices A–C end at the bottom of one page, choice D is the first thing on the next). Sometimes it's a TOTAL split: a page ends with ONLY the bare question stem/number and NO options at all — the entire options list (A, B, C, D...) starts fresh at the very top of the next page, with no repeated stem and no number in front of it. Both are NORMAL pagination, not a sign the question ended, was invalid, or wasn't a real question.
   - DO NOT use "does this page show any options near the stem" as your test for whether something is a real, extractable question. A numbered/bulleted line that reads like a question (ends in "?", or reads "Which of the following...", "All of the following EXCEPT...", "A patient presents with... What is the most likely diagnosis?", etc.) IS a question the instant you see it, even if the page ends right there with zero options visible yet. Hold it open mentally and keep reading — the next page is where you finish it, not a new document.
   - Conversely, if a page OPENS with a lettered list (A. ... B. ... C. ... D. ...) that has no question stem of its own above it on that page, that is an ORPHAN option block — it does NOT start a new question. It belongs to the last unfinished question stem from the end of the previous page. Attach it there.
   - WORKED EXAMPLE 1 (partial split): page 7 ends with "14. Which of the following is the most likely diagnosis?\nA. Addison disease\nB. Cushing syndrome" and page 8 begins with "C. Conn syndrome\nD. Pheochromocytoma\n(Answer: C)" — followed by unrelated question 15. Output ONE object for question 14 with all four options and answer "C". Outputting only A/B, marking it "__NO_KEY__", or dropping it are all WRONG.
   - WORKED EXAMPLE 2 (total split — the harder case): page 7 ends with just "15. A 60-year-old woman presents with weight gain, cold intolerance, and a firm goiter. What is the most likely diagnosis?" and NOTHING else — no options at all on page 7. Page 8 then begins with "A. Hashimoto thyroiditis\nB. Graves disease\nC. De Quervain thyroiditis\nD. Riedel thyroiditis" with no stem or number above it, then continues into unrelated question 16. The correct output for question 15 is ONE object combining the stem from page 7 with the orphan options from the top of page 8. It is WRONG to skip question 15 entirely just because page 7 alone had no options — you must recognize the stem as a real question the moment you read it, then claim the orphan option block on the next page as belonging to it.
   - The same "search the whole document, not just the local page" principle applies to answer keys: if a question's ONLY indication of the correct choice is a separate answer-key list positioned elsewhere (a different page, or a key section at the very end, e.g. "12-C, 13-A, 14-B..."), find that entry by matching the question number and use it — do not mark the question "__NO_KEY__" just because the key wasn't printed directly under the question itself.
   - A case/vignette cluster (rule 8) can itself span a page break the same way — the shared case text may end one page and the dependent questions, or even the case's own concluding sentence, continue on the next; keep them in the same cluster and never cut the case text short just because the page changed.
   - MANDATORY SELF-CHECK BEFORE YOU OUTPUT ANYTHING: this check must cover more than your draft list, because a totally-split question (Worked Example 2) may never have made it into that list at all. So: (a) re-scan every page ending for a bare stem/question-like sentence that you may have set aside because it had no visible options — if you find one, go to the start of the next page and pull its options from there; (b) re-scan every page opening for an orphan lettered list with no stem above it — if you find one that you haven't already attached to a question, it belongs to the previous page's last stem; (c) for every question already in your draft list, confirm "options" has at least 2 entries — if not, re-search neighboring pages and any answer-key section before finalizing; (d) if the question numbers in the source jump (e.g. you have 13 and 15 but not 14), go find 14 — it's usually the victim of exactly this kind of split; (e) confirm "__NO_KEY__" is only used when the key genuinely never appears anywhere in the document, not because you stopped looking after the page the question started on. Only finalize the JSON after this check.
