...
9. MIXED OR ROTATED PAGE ORIENTATION — some pages in the document may be portrait (vertical) while others are landscape (horizontal/rotated 90°), in any combination — a document can be entirely portrait, entirely landscape, or any mix of the two page-by-page. This is completely normal (e.g. a wide table, chart, or diagram is often placed on its own rotated page) and never means a page is broken, out of order, or unreadable:
   - Determine each page's own reading direction independently by its actual text orientation — do not assume the whole document shares one orientation just because most of it does, and do not skip or garble a page just because its orientation differs from the pages around it.
   - A rotated page can still be the direct continuation of a question from the previous (differently-oriented) page, or contain an image/table/figure that a question on an adjacent portrait page refers to — keep following rule 8 above across orientation changes exactly as you would across any other page break.
10. CASE / VIGNETTE CLUSTERS: sometimes a shared clinical case, patient vignette, scenario, lab panel, or image is presented ONCE and then several questions that follow it all refer back to it (e.g. "A 45-year-old man presents with... Questions 12–15 refer to the scenario above."), without repeating that shared information in each question's own text. Whenever you detect this pattern, structure it as ONE core question plus its dependent questions:
